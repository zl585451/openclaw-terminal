...
> AMY 的追问不是"功能"，是对话能力。像朋友聊天一样自然地确认意图。
...
下次遇到类似场景时，AMY 先检查 `core://my_user/preferences/` 下是否已有偏好记录：
- **有记录** → 直接给出倾向该偏好的建议，附带一句"你之前更倾向 X，这次也是吗？"
- **无记录** → 正常追问

### 6.3 与反馈闭环联动

`memory_feedback.js` 已有的反馈检测逻辑（少爷说"好"/"不对"）同样适用于追问场景：
- 追问后少爷说"问得好" → 正面反馈，记录该追问模式
- 追问后少爷说"别问了" → 负面反馈，该场景下次不追问
...
```
Round 1: AMY 追问方向 → 用户点 pill
Round 2: AMY 追问细节 → 用户点 pill 或自由回答
Round 3: AMY 给出具体建议（不再追问）
```

**旁支停车**：追问主线时发现用户提到了其他问题，用停车场模式暂存：
```
少爷，打包脚本的问题我先记下了 📌 我们先把眼前这个理清～
...
取而代之：AMY 在回复正文中自然追问 + 使用渲染标签 + 情绪感知 + 偏好学习。
...
| 信号类型 | 用户示例 | AMY 策略 |
|----------|---------|---------|
| **意图模糊** | "帮我看看这个" | 确认"看"的含义（审查？优化？修复？） |
| **隐含冲突** | "我想又快又好" | 拆解冲突，确认优先级 |
| **选择困难** | "不知道该不该升级" | 找出纠结点（钱？风险？必要性？） |
| **信息不足** | "帮我设计个方案" | 确认范围和约束 |
| **开放性问题** | "接下来做什么好" | 给框架 + 追问方向偏好 |
| **多重目标** | "我想学 Rust 又想学 Go" | 帮他聚焦一个 |
...
| 场景 | 标签 | 说明 |
|------|------|------|
| 简单选择（2-4项） | `[pills]` | 单击即发送，最轻量 |
| 多选场景 | `[checkbox]` | 勾选后确认发送 |
| 引导深度思考 | `[question]` | 点击填充到输入框 |
| 不需要追问 | 不加标签 | 正常回复 |
...
取而代之：AMY 在回复正文中自然追问 + 使用 `[pills]`/`[question]` 渲染标签。
