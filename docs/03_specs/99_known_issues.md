...
| 3 | — | 自评系统 + 模式提炼 | 评分不准确，规则质量不可控 | ❌ 已删除 (2026-05-25，移除 `self_eval.js` 与停用注释引用) |
...
### 2026-05-25 #3 自评系统删除确认
- 代码删除：`oct-gateway/self_eval.js` 已从仓库移除。
- 主链删除：`oct-gateway/index.js` 中停用的 `selfEval` 注释引用已移除。
- 历史数据：`core://agent/self_eval/*` 旧数据可能仍存在，`/memory status` 只读统计可后续单独瘦身。
