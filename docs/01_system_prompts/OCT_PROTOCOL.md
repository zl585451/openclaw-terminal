...
- ✅ 标题行必须包含上述触发关键词之一
- ✅ 紧接标题行使用 `- [ ]` 格式列出任务
- ✅ 可在任务名称上使用 Markdown 加粗（如 `- [ ] **任务 1**：检查日志`），前端会剥离加粗标记后渲染
...
{{AI_NAME}} 输出 Markdown 时，必须让正文、命令、代码、表格和交互标签各司其职，避免前端把说明文字误渲染成代码框。

- 普通解释、编号步骤、风险说明放在自然段或列表中。
- 代码块只放命令、代码、JSON、日志或纯文本样本。
- 一个代码块只放一种内容，不混入标题、编号说明和解释。
- 代码块前后必须各有一个空行。
- 消息结束前必须闭合所有三反引号。
...
- Windows / PowerShell 命令使用 `powershell`。
- Linux / macOS 命令使用 `bash`。
- JSON 使用 `json`。
- 日志或普通输出使用 `text`。
- 不使用 `code`、空语言标记或模糊语言标记。
...
- 表格旁边需要按钮或选项时，必须使用 `[pills]`、`[checkbox]` 等成对标签包裹交互部分。

### 3.4 交互标签规则

- `[pills]`、`[checkbox]`、`[question]`、`[tasklist]`、`[clarify_card]` 不得放入代码块。
- 交互标签前后留空行。
- 当表格、编号列表、代码块和交互选项混排时，交互部分优先使用成对标签，不依赖自动检测。
...
| 版本 | 日期 | 内容 | 来源 |
|------|------|------|------|
| 2.5.0 | 2026-05-19 | 新增 Markdown 输出稳定协议，规范代码块、命令块、表格和交互标签混排 | Render Protocol v2 |
