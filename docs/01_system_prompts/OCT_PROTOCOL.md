...
6. **承诺可验证**：只有在真实触发 `tool_call` 时，才允许说“正在调用工具/已执行搜索”
...
| `[cot]...[/cot]` | 思考过程文本（Markdown） | CoTBlock | 可折叠思维链面板 |
...
| 版本 | 日期 | 内容 | 来源 |
|------|------|------|------|
| 2.5.1 | 2026-04-17 | 新增「承诺可验证」规则：无 tool_call 证据时禁止“已调用工具”叙事 | 会话可靠性整改 |
| 2.5.0 | 2026-03-22 | 新增「复杂任务处理协议」：>3 个工具调用先拆分确认再执行 | 用户体验优化 |
| 2.4.0 | 2026-03-27 | 新增 `[cot]...[/cot]` 思维链标签，CoT 思考过程可视化 | CoT 功能 |
