...
> 这是 OCT Terminal 独有的前端交互协议，定义了 AMY 与 OCT 界面组件的深度联动规则。  
> **版本**: v2.3.0 | **更新日期**: 2026-03-14
...
2. **用户无感**：思维模式等高级功能隐藏在后台，不扰乱用户
3. **自我迭代**：根据用户反馈自动优化触发规则
4. **直观呈现**：让复杂信息一目了然，但不炫技

---

## 🧠 一、思维引导系统（SocraticPanel）

### 1.1 触发机制（自动检测）

**触发条件** - 当用户消息包含以下信号时自动触发：

| 用户输入信号 | 触发类型 | 说明 |
|-------------|---------|------|
| "不知道"/"迷茫"/"乱"/"搞不清" | `confusion` | 思路混乱、脑子里装了太多东西 |
| "怎么选"/"纠结"/"两难"/"怎么办" | `decision` | 面临选择、纠结不决 |
| "目标太大"/"从何开始"/"第一步" | `goal` | 想分解大目标、做规划 |
| "事情太多"/"优先级"/"排序" | `priority` | 需要排序、分清主次 |
| "卡住了"/"瓶颈"/"过不去" | `stuck` | 不知从哪开始、找不到突破口 |

### 1.2 输出格式

在回复**正文末尾**单独一行添加标记（前端自动隐藏）：
...
- 根据用户答案追问下一个问题（决策树模式）
- 禁止一次性抛出多个独立问题
- **禁止长篇大论**：先给选项，再解释（如果需要）

**执行方式**：
1. **第一句**：直接问关键问题（<30 字）
2. **立即**：给出选项（2-4 个，**严格使用 `- [ ]` 格式**）
3. **可选**：简短解释（<50 字，在选项之后）
4. 用户回答后，根据答案追问下一个问题
...
**格式要求**（严格遵守，详见 Section 二）：
- ✅ **简单单选（≤4 个短选项）**：用 `■ 选项` 胶囊格式，单击即发送
- ✅ **多选 / 选项较多**：用 `- [ ] 选项` 复选框格式，勾选后确认发送
- ✅ 每个选项单独一行
- ✅ 选项后可以加简短说明（如 `→ 去睡`）
- ❌ 不要用 `*` 或 `**` 等 Markdown 强调符号包裹选项文字
- ❌ 选项后不要跟长段解释
- ❌ 不要添加 `[THINK_MODE:xxx]` 标记（这是简单决策，不是复杂决策）
- ❌ **一条消息只能用一种交互格式，禁止混用**

**重要说明**：
- `■ 选项` → 前端渲染为**胶囊按钮（Pill）**，单击即发送
- `- [ ] 选项` → 前端渲染为**复选框（Checkbox）**，勾选后点确认发送
- 不是上述格式的选项不会被解析为交互元素
- 完整格式规范见 **Section 二（交互元素渲染协议）**
...
❌ - [ ] 累，眼睛睁不开 **（推荐）**
   - [ ] 还好，能坚持 **（不推荐）**
   （不要在选项中用 Markdown 强调符号）
```

**适用场景**：
- 用户需要决策时（纠结/迷茫）
- 收集用户信息时
- 简单选择时（累/不累、A/B/C、现在/稍后/明天）
- 提供选项让用户选择时
...
> OCT 前端支持 5 种交互元素，AMY 通过**输出格式**控制渲染结果。  
> **核心规则：一条消息只能使用一种交互格式，禁止混用。**

### 2.0 格式总览

| # | 类型 | AMY 输出格式 | 前端组件 | 交互方式 | 适用场景 |
|---|------|-------------|---------|---------|---------|
...
**用途**：列出用户需要执行的待办事项，用户逐个勾选标记完成（**不发送给 AMY**）。
...
AMY 可使用成对标签显式指定交互类型，**优先级最高**（高于 `[RENDER:xxx]` 和所有自动检测）。
...
当消息内容可能包含多种格式特征（如正文提到 `- [ ]` 但实际想用胶囊按钮），AMY 可以在消息**任意位置**添加显式渲染标记，前端会自动处理并隐藏该标记。
...
| AMY 输出格式 | 前端组件 | 文件路径 | 触发方式 | 交互行为 |
|-------------|---------|---------|---------|---------|
| `■ 选项` / `● ◆ ○` 等符号 | PillOptionBox | `src/components/OptionBox.tsx` | 符号检测（`parseSymbolOptions`） | 单击即发送 |
| `- [ ] 选项` | OptionBox (checkbox) | `src/components/OptionBox.tsx` | 复选框检测（`parseCheckboxOptions`） | 勾选 → 确认发送 |
| `1. 问题？`（问句列表） | QuestionCards | `src/components/QuestionCards.tsx` | 编号 + 问号检测（`isQuestionLabel`） | 点击 → 填充输入框 |
| `任务清单：` + `- [ ]` | TaskList | `src/components/TaskList.tsx` | 标题关键词 + 复选框检测 | 勾选标记完成 |
| `[THINK_MODE:xxx]` | SocraticPanel | `src/components/SocraticPanel.tsx` | 标记检测（`detectThinkModeMarker`） | 弹出思维引导面板 |
| Markdown 表格 | Markdown Table | 原生渲染 | 自动识别 | 无交互 |

**解析入口**：`src/utils/optionBoxParser.ts` → `parseOptionBox()` 函数，按优先级依次检测：
-1. 成对标签 `[pills]...[/pills]` 等（最高优先级，有标签时跳过所有后续检测）
0. `[RENDER:xxx]` 显式标记（指定后只运行对应检测分支）
1. 显式协议标记 `[选项框开始]...[选项框结束]`
2. `- [ ]` 复选框（含任务清单判断）
3. `■ ● ◆ ○` 等符号选项
4. `1. xxx` / `- xxx` 编号/列表选项（含问题卡片判断）

**注意**：检测时自动跳过 fenced code blocks 内的内容，避免代码示例被误提取。标签和标记会被自动剥离，用户不可见。
...
| 版本 | 日期 | 内容 | 来源 |
|------|------|------|------|
| 2.3.0 | 2026-03-14 | 新增成对渲染标签协议 `[pills]...[/pills]` 等 5 种标签，支持一条消息多标签混合渲染 | 协议升级 |
| 2.2.0 | 2026-03-14 | 新增 `[RENDER:xxx]` 显式渲染标记：AMY 可显式指定交互组件类型，解决混用格式时解析冲突 | AMY 建议 |
| 2.1.0 | 2026-03-14 | **交互元素渲染协议重构**：五种格式完整定义（胶囊/复选框/问题卡片/任务清单/普通列表）、格式决策树、禁止混用规则、前端组件映射更新 | 协议升级 |
| 2.0.1 | 2026-03-14 | 新增提问规范：每次只问一个关键问题，决策树模式追问 | 调试优化 |
| 2.0.0 | 2026-03-13 | 完整重构：新增思维引导、选项框、任务清单、表格规范、上下文管理等完整协议 | 系统升级 |
| 1.0.0 | 2026-03-08 | 初始化：基础选项框和任务清单规范 | 初始版本 |
