...
> **依赖**: SOUL.md（情绪感知）· AGENTS.md（调度规则）
...
自动反馈默认关闭。追问场景只在{{USER_NAME}}明确表达可复用偏好或纠正时写入 Memory v2：
- 追问后{{USER_NAME}}说"问得好/这个方向对" → 可记录该追问模式
- 追问后{{USER_NAME}}说"别问了/以后不要这样问" → 写入纠正规则，下次降低追问
