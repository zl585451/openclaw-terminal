...
2. 质检结论为 `reject` 时，执行器会把 Gate 标记为 `rejected`，阻止进入交付打包，并把该章标记为失败。
3. 非 `reject` 的质检结果仍会作为提示卡展示，允许继续生成交付包，但应在交付摘要中保留问题记录。
4. Gateway 断线重连场景下，批次事件通过订阅表补投递；历史遗留 `awaiting_review` 批次会被迁移，避免旧暂停状态劫持工作台视图。
