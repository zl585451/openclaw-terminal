...
| 前端设置投影 | `src/hooks/settings/useApiKeys.ts`、`src/ui/settings/tabs/ConnectionTabView.tsx` | 展示 provider/model/key，构造保存 payload，读取 OmniRoute 状态 | `FALLBACK_PROVIDERS` 与 gateway provider metadata 有重复趋势；前端不应成为 provider registry 权威 | 需优化 | `src/hooks/__tests__/settings.test.ts`，后续补“只消费后端 provider list”测试 |
