...
- 前端设置页“不维护 provider registry 权威”的结构性测试；当前业务仍有 `FALLBACK_PROVIDERS`，先记录为 Phase D 风险，不在 Phase C 强改。
