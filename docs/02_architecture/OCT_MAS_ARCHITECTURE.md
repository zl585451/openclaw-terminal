...
**一句话总结**：AMY 是前台接待，负责理解和调度；各专职 Agent 是后台专家，负责执行；共享记忆由 Memory v2 本地存储承载。

**三个原则**：
1. **不重复造轮子** — 现有的 oct-gateway 已经有 WebSocket 通信、Memory v2 记忆、Session 管理，MAS 在这个基础上叠加，不推翻重写
