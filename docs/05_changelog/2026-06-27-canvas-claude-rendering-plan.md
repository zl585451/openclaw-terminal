...
### Phase 1 — 通用点击解释层（核心）
- [ ] `artifactShell.ts`：注入脚本增加点击监听——冒泡找最近 `[data-explain]`（兜底 `.node`/带文字 `<rect>`），
      取 label → `postMessage({__octArtifactInspect:true, label})`；注入 `[data-explain]{cursor:pointer}` + hover 高亮。
- [ ] `htmlPlugin.tsx`：组件内 `useCanvas()` 拿 `onNodeInspect`；监听 `__octArtifactInspect` 消息 → 调用之。
- [ ] 验收：SVG 架构图里点节点 → 聊天框自动发"请详细说明节点「X」…"。

### Phase 2 — 系统协议收口（contextBuilder）
- [ ] ui-draft 协议新增：每个可解释节点加 `data-explain="节点名"` 属性。
- [ ] 关键词没命中的**默认产图 fallback** 指向 ui-draft（SVG）协议，杜绝裸奔选 react-flow。
- [ ] react-flow 协议改为窄场景说明（仅"可拖拽/缩放探索的大型交互图"）。

### Phase 3 — 路由收口（orchestrator + canvas 工具）
- [ ] `CANVAS_TRIGGER_RULES` ui-draft 关键词扩充：工作流程/数据流/调用链/pipeline/时序流程…
- [ ] `tools/canvas.js`：react-flow 的 enum 描述收窄为明确窄场景。

### Phase 4 — 渲染优先级确认
- [ ] 核对 `plugins/index.ts` 解析顺序，确保普通结构/流程图稳定走 htmlPlugin。

### Phase 5 — react-flow 降级保留
- [ ] 暂不删除、暂不修布局；仅确保默认不再被触发。后续如仍出现再单独处理。
