...
- provider 提示文案、模型输入模式、自定义 provider 扩展区显隐已开始改用 view schema
- `ConnectionTabView.tsx` 中新增 provider 所需的主表单条件分支已基本收口到 schema 层
- Exit Criteria:
...
- 已新增 `ui/TaskCreateWizardSidebar.tsx`
- 已新增 `ui/TaskCreateWizardFooter.tsx`
- `TaskCreateWizard` 现已具备容器 + hook/policy + view shell 分层
- Exit Criteria:
