...
| **B3 切换段渲染** | ⏸ 待联调 | 改动核心流式渲染（打字机 DOM/markdown 生长/CoT 折叠），**必须在运行的应用里肉眼验证**滚动/动画后才能切。需用户在场。 |
| **B4 删补丁 + 统一 TurnOutcome** | ⏸ 前置 B3 | 计划自身要求"B3 灰度确认稳定"后才删旧路径。 |

> **当前安全边界**：B0–B2 全部有测试、零回归、已提交。B3 是可见渲染切换，按本方案 §6 风险与项目"有风险先告知"原则，**不在无人值守下盲切生产渲染**——需用户运行应用联调确认。
