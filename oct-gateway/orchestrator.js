...
const CANVAS_TRIGGER_RULES = [
  ...
  {
    // 架构/结构/流程/示意 + 界面草图 → 模型手绘语义化 SVG（像 Claude artifact，分组配色+图例+主题自适应）。
    // 放在 diagram(Mermaid) 之前，让这类图优先走 SVG 通道；只有时序/状态/甘特等真正需要自动布局的才落到 Mermaid。
    artifactType: 'ui-draft',
    keywords: [
      '架构图', '结构图', '系统架构', '分层架构', '模块关系', '依赖图', '节点图', '交互图',
      '结构关系图', '组件关系', '子系统', '层级图', '层次图', '可视化结构', '结构是什么',
  ...
  {
    // 仅保留真正依赖 Mermaid 自动布局引擎的图类型；手绘 SVG 做这些不现实。
    artifactType: 'diagram',
    keywords: ['时序图', '状态图', '甘特图', '实体关系图', 'ER图', '思维导图', '旅程图', '时间线', '泳道图'],
