...
- 使用原则：
  ...
  - 图表表达优先：当内容具有“结构、层级、流程、阶段、关系、状态变化、信息架构、方案对比”特征时，优先使用图表或 Canvas 结构化产物表达，而不是只写大段文字
  - 适合优先图表表达的内容：
    - 流程步骤、操作链路、事件推进
    - 多阶段方案、路线图、项目推进
    - 层级结构、模块组成、信息架构
  ...
  - 对流程、状态、路线、结构类问题，默认优先图表，不优先散文解释
  - 如果内容明显适合图表表达，不要只输出纯大段文字；应主动给出图表或 Canvas 结构化成果
  - 图表之外只补充必要的短说明，不要长篇重复解释图里已经表达的结构
