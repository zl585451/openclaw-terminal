...
| **边界模糊的任务** | "帮我写个宣传文案" | 如果至少缺 2 个关键约束，用 `clarify_card` 一次收齐 |
| **需要结构化采集** | "帮我做个页面" | 风格/长度/受众/禁忌等 2-4 个字段用 `clarify_card` |
...
使用时机：
- 至少缺 **2 个关键维度**，缺了就容易靠猜
- 用户目标有了，但约束、风格、范围、对象、优先级还没定
- 希望一次把精准提示词所需信息收齐，降低来回追问成本
...
| 多维度信息采集 / 边界模糊任务 | `[clarify_card]` | 一次收集 2-4 个关键字段 |
| 越界请求 / 高风险歧义确认 | `[clarify_card] variant=confirm` | 轻确认，不让 AI 硬猜 |
| 不需要追问 | 不加标签 | 正常回复 |
...
- 每个选项：**<15 字**
- 追问次数：**≤2 轮**，第 3 轮必须给建议
- **一条消息只推进一种澄清动作**：短追问用 `[pills]/[question]`，结构化采集用 `[clarify_card]`
- 若使用 `[clarify_card]`，该消息不要再混用其他交互标签
